...
BAD_TITLES = {
    "MARK", "QTY", "FT IN", "WEIGHT", "MATERIAL", "DATE", "REV", "CHECKED", "DRAWN", "PROJ", "CONTRACTOR", "PROJECT", 
    "OWNER", "DESCRIPTION", "CLIENT", "TO BE", "SHALL BE", "U.N.O.", "U.N.O", "UNLESS NOTED", "RADIUSED", "SEE DETAIL",
...
def is_valid_title_candidate(s: str) -> bool:
    ...
    if BODY_NOTE_PATTERN.match(s_up):
        return False
    # Exclude addresses (look for street types or state zip patterns)
    if re.search(r'\b(?:ROAD|DRIVE|VALLEY|STREET|AVE|ST|RD)\b', s_up):
        return False
    if re.search(r'\b(?:ADDRESS|PROJECT ADDRESS)\b', s_up):
        return False
    # Exclude states at end of line (e.g. HOWARD, PA)
    if re.search(r',\s*(?:PA|TX|CA|NY|FL|GA|IL|OH|VA|NC|NJ|MA)\s*$', s_up):
        return False
    # Title shouldn't be too long (Address/Notes are usually long)
    if len(s.strip()) > 70:
        return False
    # Exclude sequences of more than 8 words (likely a sentence/note)
    if len(s.split()) > 8:
        return False
    # --- 1. Hard EXCLUSIONS (The common stuff) ---
    if any(bad in s_up for bad in BAD_TITLES):
        return False
    
    # Exclude purely instruction or status phrases (Substring search)
    STATUS_PHRASES = ["FOR FABRICATION", "FOR APPROVAL", "FOR CONSTRUCTION", "ISSUED FOR", "RECORD DRAWING", "PRELIMINARY"]
    if any(status in s_up for status in STATUS_PHRASES):
        return False

    # Exclude numeric-heavy strings or purely code-like strings (e.g. 25012)
    # If the title is > 40% digits, it's probably junk or a date code
    digit_count = sum(c.isdigit() for c in s_up)
    if len(s_up) > 0 and (digit_count / len(s_up)) > 0.4:
        return False

    # Exclude legal and instruction phrases
    if any(bad in s_up for bad in ["CONSENT", "WRITTEN", "WITHOUT", "PROPERTY", "COPYRIGHT", "PERMISSION"]):
        return False
    if re.search(r'^(?:SEE|PER|DO NOT|MUST|WILL|ALL|USE|NOTE|NOTED)\b', s_up):
        return False
    # Exclude strings that end with a colon (These are labels, not values)
    if s.strip().endswith(":"):
        return False
    # Exclude process verbs that indicate status rather than naming
    if re.search(r'\b(?:ISSUED|REVISED|APPROVED|CHECKED|DRAWN|SHIPPED|FABRICATED|RECORD|COPIES)\b', s_up):
        return False
    # Exclude URLs and web addresses
    if re.search(r'\b(?:WWW\.|HTTP|HTTPS|\.COM|\.CO\.IN|\.NET|\.ORG)\b', s_up):
        return False
...
def score_title(s: str) -> int:
    ...
    if keyword_matches:
        current_score += 5000  # Extreme reward for structural nouns
        current_score += len(keyword_matches) * 200
        # Extra reward if the title STARTS with or IS exactly a keyword
        if any(s_up.startswith(k) for k in keyword_matches):
            current_score += 1000
            
    if "DETAIL" in s_up or "PLAN" in s_up:
        current_score += 500
    ...
    noise_words = [' PER ', ' OF ', ' FOR ', ' PLAN ', ' BY ', ' NO.', ' REF ', ' DWG ', ' SHEET ', ' RADIUSED ', ' TO BE ', ' SHALL BE ']
    current_score -= sum(120 for noise in noise_words if noise in s_up)
...
def extract_locally_pass(pdf_path: str, original_filename: str = "", extraction_mode: str = "layout") -> dict:
    ...
    try:
        with pdfplumber.open(pdf_path) as pdf:
            ...
            # --- 1. Drawing Number ---
            target_filename = original_filename if original_filename else os.path.basename(pdf_path)
            fn_base = target_filename.split('_')
            ...
            if not fields["drawingNumber"]:
                ...
                fields["drawingNumber"] = pl_match.group(1) if pl_match else (fn_hint or target_filename.replace(".pdf", ""))
            ...
            # Labels to check in any case (DWG DESCRIPTION, SHEET TITLE, CONTENTS, etc.)
            TITLE_LABELS = ['DWG', 'SHEET', 'DRAWING']
            TITLE_TYPES = ['DESCRIPTION', 'TITLE', 'NAME', 'CONTENTS']
            
            # Check for inline labels anywhere in text
            for line in all_lines:
                # Build a dynamic regex for all combinations: (DWG|SHEET) (DESCRIPTION|TITLE|NAME)
                label_pattern = fr'\b(?:{"|".join(TITLE_LABELS)})\s+(?:{"|".join(TITLE_TYPES)})\s*[:.\-\s]*'
                is_label = re.search(label_pattern, line, re.I)
                if is_label:
                    # Capture everything after the label on the same line
                    m = re.search(fr'{label_pattern}\n?\s*(.+)', line, re.I)
                    if m:
            ...
            if not explicit_title:
                label_pattern = fr'(?:{"|".join(TITLE_LABELS)})\s+(?:{"|".join(TITLE_TYPES)})\s*[:.\-\s]*'
                m2 = re.search(
                    fr'{label_pattern}\n?\s*([A-Z0-9\s,&/\-]{{3,80}})(?=\s\s+|\n|[A-Z]{{3,}}:|$)',
                    clean_text, re.I
                )
            ...
            # Drawing Title / Sheet Title label (Mixed Case or Upper)
            if not explicit_title:
                m1 = re.search(r'(?:Drawing|Sheet|DWG)\s*(?:Title|Name|Description)\s*[:.\-\s]+(\S[^\n]*)', clean_text, re.I)
                if m1:
            ...
            # ── Fallback 3: Proximity search ──
            # Re-run title detection if still empty, looking near the Project Name
            # MUST match a keyword OR be very prominent in the bottom right to be accepted
            if not fields["drawingTitle"]:
                for i, line in enumerate(all_lines):
                    if fields["projectName"] and fields["projectName"] in line:
                        for offset in range(-3, 6): # Check lines above/below project name
                            target_idx = i + offset
                            if 0 <= target_idx < len(all_lines):
                                potential = all_lines[target_idx].strip()
                                if is_valid_title_candidate(potential) and not HARD_STOP_PATTERN.match(potential):
                                    potential = strip_all_dates(potential)
                                    # Accept if it's a keyword OR matching drawing number prefix
                                    # (e.g. if dwg starts with RB, look for ROUNDBAR)
                                    up_potential = potential.upper()
                                    if any(kw in up_potential for kw in KEYWORDS) or \
                                       (fields["drawingNumber"].startswith("RB") and "ROUND" in up_potential) or \
                                       (len(potential) > 5 and potential.isupper() and ' ' in potential):
                                        fields["drawingTitle"] = potential
                                        fields["drawingDescription"] = potential
                                        break
                        if fields["drawingTitle"]: break
                
            # ── Fallback 4: "Hotzone" Scan (Bottom-Right 20% by word density) ──
            # If still empty, look for the most "Title-like" candidate in the last few lines
            if not fields["drawingTitle"]:
                # Candidates from the very end of the file (usually the title block in text streams)
                for line in reversed(all_lines[-15:]):
                    candidate = line.strip()
                    if is_valid_title_candidate(candidate) and any(kw in candidate.upper() for kw in KEYWORDS):
                        fields["drawingTitle"] = candidate
                        fields["drawingDescription"] = candidate
                        break
...
def extract_locally(pdf_path: str, original_filename: str = "") -> dict:
    """
    ...
    # --- Stage 1: Layout ---
    fields = extract_locally_pass(pdf_path, original_filename=original_filename, extraction_mode="layout")
    
    # --- Stage 2 & 3: Geometric & Table (Interleaved in pass logic) ---
    if is_incomplete(fields):
        f_geo = extract_locally_pass(pdf_path, original_filename=original_filename, extraction_mode="geometric")
    ...
    if is_incomplete(fields):
        try:
            f_ocr = extract_locally_pass(pdf_path, original_filename=original_filename, extraction_mode="ocr")
            for k in ["drawingNumber", "drawingTitle", "revision", "date", "remarks", "revisionHistory"]:
    ...
    if is_incomplete(fields):
        f_raw = extract_locally_pass(pdf_path, original_filename=original_filename, extraction_mode="raw")
        for k in ["drawingNumber", "drawingTitle", "revision", "date", "remarks", "revisionHistory"]:
    ...
    if not fields.get("drawingDescription") and fields.get("drawingTitle"):
        fields["drawingDescription"] = fields["drawingTitle"]
...
def main():
    ...
    try:
        raw_dict = extract_locally(pdf_path, original_filename=args.original_filename)
        fields = {
