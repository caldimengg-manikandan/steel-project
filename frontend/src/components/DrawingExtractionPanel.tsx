...
interface DrawingExtractionPanelProps {
    ...
    sequences?: any[];
    projectYear?: number;
    startingTransmittalNumber?: number;
}
    ...
    async function processUploads(fileArray: File[]) {
        ...
        if (transmittalNumberToUse === null) {
            ...
            if (isOldProject && manualTransmittalNumber && Number(manualTransmittalNumber) >= 1) {
                ...
                // Reserve the next number from the DB counter
                try {
        ...
        <div>
            ...
            <div className="panel-status-header" style={{
                ...
                <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
                    ...
                    {completedCount > 0 && (
                        <>
                            {/* Download buttons moved to Storage panel */}
                        </>
                    )}
                </div>
            </div>
                    ...
                    <div className="modal" style={{ maxWidth: 500 }} onClick={(e) => e.stopPropagation()}>
                        ...
                        <div className="modal-body">
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 20 }}>
                                ...
                                >
                                    <div style={{ display: 'flex', alignItems: 'center', gap: 12, width: '100%', cursor: 'pointer' }} onClick={() => setSelectedTransmittalNumber(null)}>
                                        <input
                                            type="radio"
                                            name="transmittalChoiceDEP"
                                    ...
                                    {selectedTransmittalNumber === null && projectYear && projectYear < 2026 && (
                                        <div style={{ marginTop: 12, width: '100%', paddingLeft: 28 }} onClick={(e) => e.stopPropagation()}>
                                            ...
                                            <input
                                                ...
                                                min="1"
                                                placeholder="e.g. 13"
                                                value={manualTransmittalNumber}
                                                onChange={(e) => setManualTransmittalNumber(e.target.value)}
                                                style={{ fontSize: 14, fontWeight: 700, maxWidth: 160, border: '2px solid #059669' }}
                                            />
                                            <p style={{ fontSize: 11, color: '#9a3412', marginTop: 6 }}>
                                                Enter the transmittal number this upload should generate (e.g. TR-{(manualTransmittalNumber || '?').toString().padStart(3, '0')}).
                                            </p>
                                        </div>
                                    )}
                                </label>
