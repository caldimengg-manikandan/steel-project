                ...
                if (fetchedProjects.length > 0) {
                    ...
                    if (stateProjectId) {
                        ...
                        if (target) {
                            ...
                            setSelectedSequences([]); // Reset sequences when switching project
                        } else {
                            setSelectedProject(fetchedProjects[0]);
                            setSelectedSequences([]); // Reset sequences
                        }
                    } else {
                        setSelectedProject(fetchedProjects[0]);
                        setSelectedSequences([]); // Reset sequences
                        ...
                        <div style={{ display: 'flex', gap: 8 }}>
                            ...
                            <a
                                ...
                                style={{
                                    ...
                                    textDecoration: 'none', boxShadow: '0 2px 8px rgba(100,116,139,0.3)',
                                    transition: 'opacity 0.15s',
                                }}
                            >
                                <IconDownload /> Download Closed RFI
                            </a>
                        </div>
                    </div>
                )}
                                ...
                                return (
                                    <button
                                        ...
                                        onClick={() => { 
                                            ...
                                            setUploadError(''); 
                                            setUploadSuccess(''); 
                                            setSelectedSequences([]); // Reset selection
                                        }}
                                        style={{
                                    ...
                                    <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginTop: 10 }}>
                                        ...
                                        <button
                                            ...
                                            style={{
                                                ...
                                                background: uploading ? '#e5e7eb' : 'linear-gradient(135deg,#2563eb,#7c3aed)',
                                                color: uploading ? '#9ca3af' : 'white',
                                                fontSize: 13, fontWeight: 600, display: 'flex', alignItems: 'center', gap: 8,
                                                transition: 'all 0.15s',
                                                boxShadow: uploading ? 'none' : '0 2px 6px rgba(37,99,235,0.25)',
                                            }}
                                        >
                                        ...
                                        {sequenceFilter ? (
                                            ...
                                            (() => {
                                                ...
                                                const gridTemplate = hasAnyClientRfi 
                                                    ? '85px 15px 1.2fr 110px 110px 140px 110px 105px 30px' 
                                                    : '85px 15px 1.2fr 110px 140px 110px 105px 30px';
                                                                    ...
                                                                    <div onClick={() => setExpanded(isExp ? null : key)} style={{ display: 'grid', gridTemplateColumns: gridTemplate, padding: '14px 20px', cursor: 'pointer', alignItems: 'center', background: isExp ? 'rgba(37,99,235,0.04)' : 'transparent' }}>
                                                                        <div style={{ display: 'flex', alignItems: 'center', gap: 10, overflow: 'hidden' }}>
                                                                            ...
                                                                            <div style={{ display: 'flex', alignItems: 'center', gap: 6, minWidth: 0 }}>
                                                                                <a
                                                                                    href={getRfiViewPdfUrl(String(selectedProject._id || selectedProject.id), parent._id)}
                                                                                    target="_blank"
                                                                                    rel="noopener noreferrer"
                                                                                    onClick={(e) => e.stopPropagation()}
                                                                                    style={{ fontSize: 13, fontWeight: 700, color: '#2563eb', whiteSpace: 'nowrap', textDecoration: 'underline' }}
                                                                                >
                                                                                    {rfi.rfiNumber}
                                                                                </a>
                                                                                {/* No colon after SK# here, or keep it if preferred */}
                                                                            </div>
                                                                        </div>
                                                                        <span /> {/* Spacer for the Description column if I split it */}
                                                                        ...
                                                                        {ext.rfis.map((rfi: any, i: number) => {
                                                                            ...
                                                                            return (
                                                                                <div key={i} style={{ background: 'white', border: '1px solid #e2e8f0', borderRadius: 10, padding: 16, display: 'grid', gridTemplateColumns: '70px 1.2fr 130px 1fr 1fr 80px', gap: 14, alignItems: 'start' }}>
                                                                                    <a
                                                                                        href={getRfiViewPdfUrl(String(selectedProject._id || selectedProject.id), ext._id)}
                                                                                        target="_blank"
                                                                                        rel="noopener noreferrer"
                                                                                        style={{ 
                                                                                            fontSize: 12, fontWeight: 800, background: '#2563eb', color: 'white', 
                                                                                            borderRadius: 6, padding: '4px 8px', textAlign: 'center', display: 'block',
                                                                                            textDecoration: 'none'
                                                                                        }}
                                                                                    >
                                                                                        {rfi.rfiNumber}
