...
function resolveSowKey(fullPath, validSowNames, sowPrefixes) {
    ...
    for (let i = parts.length - 2; i >= 0; i--) {
        ...
        for (const prefix of sortedPrefixes) {
            const escapedPrefix = prefix.replace(/[.*+?^${}()|[\]\\]/g, '\\const commonBorderStyle = {');
...
async function generateTransmittalExcel(transmittal, projectDetails, logoPath) {
    ...
    const sortedMains = Object.keys(groupedByMain).sort((a, b) => a.localeCompare(b));
    let slNo = 1;

    sortedMains.forEach(mainCategory => {
        if (mainCategory) {
            // Yellow Main Category Header Row (e.g. DETAIL SHEET)
            const cRow = trSheet.addRow([mainCategory.toUpperCase()]);
            const cNum = cRow.number;
            cRow.height = 22;
            ...
            trSheet.mergeCells(cNum, 1, cNum, 6);
            for (let i = 1; i <= 6; i++) {
                cRow.getCell(i).border = commonBorderStyle;
                if (i > 1) cRow.getCell(i).fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFFFFF00' } };
            }
        }

        const sortedSows = Object.keys(groupedByMain[mainCategory]).sort((a, b) => a.localeCompare(b, undefined, { numeric: true, sensitivity: 'base' }));
        ...
        sortedSows.forEach(sowKey => {
            if (sowKey) {
                ...
                sRow.getCell(1).style = {
                    font: { bold: true, size: 11, color: { argb: 'FFED7D31' } }, // Orange color
                    alignment: { horizontal: 'center', vertical: 'middle' },
                };
                trSheet.mergeCells(sNum, 1, sNum, 6);
            }

            const sortedDrawings = [...groupedByMain[mainCategory][sowKey]].sort((a, b) =>
