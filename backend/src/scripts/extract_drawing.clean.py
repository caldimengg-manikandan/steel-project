...
def normalize_date_string(date_str):
    ...
    ds = re.sub(r'\s+', ' ', ds) # normalize whitespace

    # 1. Textual MMM DD YYYY or DD MMM YYYY
    m1 = re.search(r'([A-Z]{3,})\s+(\d{1,2})[\s,]+(\d{4})', ds, re.I)
    if not m1: 
        m1 = re.search(r'(\d{1,2})\s+([A-Z]{3,})[\s,]+(\d{4})', ds, re.I)
        if m1:
            # swap if it was DD MMM
            day_val, month_val, year_val = m1.group(1), m1.group(2), m1.group(3)
        else:
            day_val, month_val, year_val = None, None, None
    else:
        month_val, day_val, year_val = m1.group(1), m1.group(2), m1.group(3)

    if month_val and day_val and year_val:
        months = {'JAN':'01','FEB':'02','MAR':'03','APR':'04','MAY':'05','JUN':'06',
                  'JUL':'07','AUG':'08','SEP':'09','OCT':'10','NOV':'11','DEC':'12'}
        ma = str(month_val)[:3].upper()
        if ma in months:
...
def extract_locally_pass(pdf_path: str, extraction_mode: str = "layout") -> dict:
    ...
    try:
        with pdfplumber.open(pdf_path) as pdf:
            ...
            # --- 1b. Remarks/Status & Title via Fitz (Fallback/Augmentation) ---
            if not fields.get("remarks"):
            ...
            if not fields.get("drawingTitle") and blocks:
                candidates = []
                for blk in blocks:
